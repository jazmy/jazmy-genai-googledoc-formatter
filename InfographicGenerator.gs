/**
 * Infographic Generator for Google Docs
 *
...
Reproduce the logo exactly as shown in the reference image.
...
function onOpen() {
  ...
  ui.createMenu('🎨 Infographic Generator')
    .addSubMenu(ui.createMenu('Generate')
      .addItem('Smart Analysis (Recommended)', 'generateSmartInfographics')
      .addItem('Per-Section Only', 'generatePerSectionInfographics')
...
function generateSmartInfographics() {
  ...
  try {
    ...
    const infographicPlan = analyzeDocumentHolistically(sections, fullText, apiKey);
    Logger.log(`Infographic plan: ${JSON.stringify(infographicPlan)}`);

    if (infographicPlan.infographics.length === 0) {
      ui.alert('Analysis Complete', 'No infographic opportunities identified.', ui.ButtonSet.OK);
      return;
    }

    // Process all planned infographics
    const results = processInfographicPlan(doc, sections, infographicPlan, apiKey);

    ui.alert('Complete',
      `Generated ${results.success} infographic(s).\n` +
...
/**
 * Generate infographics for individual sections only (original behavior)
 */
function generatePerSectionInfographics() {
  ...
  ui.alert('Processing',
    'Analyzing individual sections for infographic opportunities...',
    ui.ButtonSet.OK);
  ...
  try {
    ...
    const sectionsNeedingVisuals = analyzeSectionsForVisuals(sections, apiKey);
    Logger.log(`${sectionsNeedingVisuals.length} sections identified for infographics`);

    if (sectionsNeedingVisuals.length === 0) {
      ui.alert('Analysis Complete',
        'No sections were identified as benefiting from infographics.',
        ui.ButtonSet.OK);
      return;
    }

    const results = processInfographics(doc, sectionsNeedingVisuals, apiKey);

    ui.alert('Complete',
      `Successfully generated ${results.success} infographic(s).\n` +
      (results.failed > 0 ? `Failed: ${results.failed}` : ''),
      ui.ButtonSet.OK);
...
function analyzeSectionsForVisuals(sections, apiKey) {
  ...
  const sectionsToAnalyze = sections.slice(0, CONFIG.MAX_SECTIONS_TO_PROCESS);
...
- Maximum ${CONFIG.MAX_SECTIONS_TO_PROCESS} infographics total
...
/**
 * Processes the holistic infographic plan
 */
...
This tool analyzes your document and automatically generates
infographics using Google Gemini's Nano Banana Pro model.

GENERATION MODES:

• Smart Analysis (Recommended)
  Holistically analyzes the document and creates:
  - Document-wide summaries (for cohesive documents)
  - Multi-section infographics (for related content)
  - Single-section infographics (for standalone data)

• Per-Section Only
  Analyzes each section independently

• Document Summary Only
  Creates one executive overview infographic
...
TIPS:
• Use clear headings to define sections
• Preview before generating to review the plan
• Multi-section infographics work best when
  headings indicate related content (e.g.,
  "Phase 1", "Phase 2" or "Problem", "Solution")
